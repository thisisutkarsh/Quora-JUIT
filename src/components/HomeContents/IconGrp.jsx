...
export const IconGrp = ({
  ...
  //   For downvotes
  const [beforeM, setBeforeM] = useState(vote_count_al);
  const [flag1, setFlag1] = useState(false);
  const [downvotes, setDownvotes] = useState(false);
  const [countDown, setCountDown] = useState(
    vote_count_al === beforeM ? vote_count_al : beforeM
  );
  //   setBeforeM = 0;
  ...
  const handleClickM = async () => {
    setFlag1(true);
    downvotes ? setCountDown((e) => e - 1) : setCountDown((e) => e + 1);
    setDownvotes(!downvotes);
    setBeforeM(countDown);
  };
    ...
    <>
      <Icongroup>
        <div className="leftIcons">
          ...
          <div>
            <Button onClick={handleClickM} className="rightbtn">
              {downvotes ? <DownvoteFilled /> : <DownVote />}{" "}
              <div>{countDown}</div>
            </Button>
...
const DownVote = styled(BiDownvote)`
  font-size: 1.13rem;
`;
const DownvoteFilled = styled(GoArrowDown)`
  /* padding-left: 5px;
    padding-top: 2px;
    padding-bottom: 2px; */
  font-size: 1.55rem;
